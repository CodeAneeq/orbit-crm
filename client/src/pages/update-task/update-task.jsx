...
const UpdateTask = () => {
    ...
    const updateTaskAPI = async () => {
        ...
        try {
            ...
            let data = await axios.put(`${baseURL}/task/api/update-task/${id}`, formData, {
                headers: {
            ...
            if (data.data.status == "success") {
                setTxt("task updated successfully");
                setShowToaster(true);
            }
        } catch (error) {
            console.log(error);
    ...
    return (
        <AdminLayout>
            <div className='w-full pr-10 my-10 max-sm:pr-5'>
                ...
                <div className='flex flex-col gap-5'>
                    ...
                    <div>
                        ...
                        <br />
                        <input type="file" multiple className='border-1 max-sm:w-70 border-gray-200 p-2 rounded-lg w-80' onChange={(e) => setAttachment([...e.target.files])} />
                    </div>
                    <div>
                        <label>Select Client</label>
                        <br />
                        <select name="clients" id="clients" value={clientId} className='w-80  max-sm:w-70 mt-3 border-1 border-gray-200 rounded-lg p-2' onChange={(e) => setClientId(e.target.value)}>
                            {
                                clients.map((item) => (
                    ...
                    <div>
                        ...
                        <br />
                        <select name="member" value={memberId} id="member" className='w-80  max-sm:w-70 mt-3 border-1 border-gray-200 rounded-lg p-2' onChange={(e) => setMemberId(e.target.value)}>
                            {
                                users.map((item) => (
                                    <option value={item._id} key={item._id}>
                                        {item.name}
            ...
            <div className='flex justify-end gap-5 mt-5'>
                ...
                <SquareBtn className={"bg-blue-500 text-white"} loading={loading} txt={"Save"} onClick={() => updateTaskAPI()} />
            </div>
        </AdminLayout>
        // </div>
    )
