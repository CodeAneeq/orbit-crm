...
const AssignClient = () => {
    ...
    const assignClientApi = async () => {
        const token = localStorage.getItem("token");
        setLoading(true)
        try {
            let payload = { assignedTo: memberName };
            const res = await axios.put(`${baseURL}/client/api/assign-client/${id}`, payload, {
                headers: {
            ...
            if (res.data.status == "success") {
                setTxt("client assigned successfully");
                setShowToaster(true);
            }
        } catch (error) {
            console.log(error);
    ...
    return (
        <AdminLayout>
            <div className='w-full pr-10 my-10 max-sm:pr-5'>
                ...
                <p><span className='text-gray-500'>Client /</span> Assign Client</p>
                <h2 className='text-3xl font-medium mt-7'>Assign Client</h2>
                <p className='text-xsm text-gray-500 mt-2'>View and manage client assign</p>
                ...
                <div className='flex justify-end gap-5 mt-5'>
                    ...
                    <SquareBtn className={"bg-blue-500 text-white"} txt={"Save"} loading={loading} onClick={() => assignClientApi()} />
                </div>
            </div>
        </AdminLayout>
    )
